...
class ReportController extends Controller
{
    public function index(Request $request): Response
    {
        ...
        $tasks = Task::with("timeLogs", "assignedToUser", "project")
            ->whereNotNull("assigned_to_user_id")
            ->when($request->projects, fn($query) => $query->whereIn('project_id', $request->projects))
            ->when($request->users, fn($query) => $query->whereIn('assigned_to_user_id', $request->users));

        if ($request->dateRange) {
            $tasks = $tasks->whereHas('timeLogs', function ($query) use ($request) {
            ...
            $tasks = $tasks->whereHas('timeLogs', fn($query) => $query->where('created_at', '>', now()->startOfMonth()));
        }
        ...
        return Inertia::render('Reports/LoggedTimeSum', [
            'projects' => DB::table('time_logs')
                ...
                ->when(
                    ...
                    fn($query) => $query->where('time_logs.created_at', '>', now()->startOfMonth())
                )
                // ->when(
                //     !$request->has('everything') || $request->get('everything') !== 'true',
                //     function ($query) use ($request) {
                //         $completed = $request->get('completed', 'true') === 'true';
                //         $query->{$completed ? 'whereNotNull' : 'whereNull'}('tasks.completed_at')
